...
from pwcnet.correlation_package.correlation import Correlation
import numpy as np
...
class PWCDCNet(nn.Module):
    ...
    #     if x.is_cuda:
    #         grid = grid.cuda()
    #     vgrid = Variable(grid).to(x.device) + flo
    #
    #     # scale grid to [-1,1]
    ...
    #     mask = torch.autograd.Variable(torch.ones(x.size())).cuda(x.device)
    ...
    def forward(self,image1, image2):
        ...
        warp = SpatialTransformer(c25.shape[-2:]).to(c25.device)
        warp5 = warp(c25, up_flow6*2)
        ...
        warp = SpatialTransformer(c24.shape[-2:]).to(c24.device)
        warp4 = warp(c24, up_flow5*2)
        ...
        warp = SpatialTransformer(c23.shape[-2:]).to(c23.device)
        warp3 = warp(c23, up_flow4*2)
        ...
        warp = SpatialTransformer(c22.shape[-2:]).to(c22.device)
        warp2 = warp(c22, up_flow3*2)
        ...
        inter = self.dc_conv6(self.dc_conv5(x))
        flow2 = flow2 + self.dc_conv7(inter)

        flow6 = F.interpolate(flow6, scale_factor=64, mode='bilinear', align_corners=False) * 64
        flow5 = F.interpolate(flow5, scale_factor=32, mode='bilinear', align_corners=False) * 32
        flow4 = F.interpolate(flow4, scale_factor=16, mode='bilinear', align_corners=False) * 16
        flow3 = F.interpolate(flow3, scale_factor=8, mode='bilinear', align_corners=False) * 8
        flow2 = F.interpolate(flow2, scale_factor=4, mode='bilinear', align_corners=False) * 4

        # return [flow2, flow3, flow4, flow5, flow6], [inter2, inter3, inter4, inter5, inter6], flow
        # return [flow2], [inter2, inter3, inter4, inter5, inter6], flow
        # return flow2, inter, flow
        return [flow2]
