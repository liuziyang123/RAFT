...
class PWCDCNet(nn.Module):
    ...
    def forward(self, im1, im2, training=True):
        ...
        im1 = im1 / MAX_IMAGE
        im2 = im2 / MAX_IMAGE
        padder = InputPadder(im1.shape[-2:])
        ...
        if training:
            return padder.unpad(flow6), padder.unpad(flow5), padder.unpad(flow4), padder.unpad(flow3), padder.unpad(
                flow2)
        else:
            return padder.unpad(flow2)
